...
object SystemPromptBuilder {
    ...
    fun build(
        ...
        if (mode == "agent") {
            ...
            sb.appendLine("| read_local_file | Read a file from app internal storage, or 'list' to see files | Moderate |")
            ...
            sb.appendLine("- **Local file reading**: Use `read_local_file` to inspect app logs, exported data, and configuration files. Path is relative to app's internal storage.")
            ...
            sb.appendLine("- **If the user asks to read a local file**: Use `read_local_file` with path='list' to see available files, then path='specific/path' to read contents. Only files within the app's internal storage are accessible.")
            sb.appendLine()
