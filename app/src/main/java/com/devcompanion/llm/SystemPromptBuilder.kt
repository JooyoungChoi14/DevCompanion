...
object SystemPromptBuilder {
    ...
    fun build(
        ...
        if (mode == "agent") {
            ...
            sb.appendLine("| download_file | Download a file from URL to device Downloads folder | Sensitive (needs confirmation) |")
            sb.appendLine("| list_page_resources | List all resources loaded by the current page (scripts, CSS, images, etc) | Moderate |")
            sb.appendLine("| read_page_source | Read source code of a page resource (JS, CSS, HTML) | Moderate |")
            ...
            sb.appendLine("- **File downloads now supported**: Use the `download_file` tool to save URLs to the device Downloads folder. This is the ONLY reliable way to download files — eval_js cannot trigger real downloads.")
            sb.appendLine("- **Page resources**: Use `list_page_resources` to see all resources loaded by the current page (scripts, stylesheets, images, fonts, XHR). Use `read_page_source` to read the source code of a specific same-origin resource. Binary resources (images, fonts) return metadata only. Cross-origin resources cannot be read due to CORS/SOP restrictions.")
            ...
            sb.appendLine("- **If the user asks to download/export data**: Use the `download_file` tool to save files to the device Downloads folder. For page content, use `extract_text` or `get_dom` to collect data, then either include it in your response or use `download_file` to save it as a file. Do NOT try to trigger browser downloads via eval_js.")
            sb.appendLine("- **If the user asks about page resources**: Use `list_page_resources` to see what the current page loads, then `read_page_source` to read specific same-origin resource content. Note: cross-origin resources cannot be read due to browser security (CORS/SOP). This is useful for debugging network issues, checking which scripts/CSS are loaded, or inspecting source code.")
            sb.appendLine()
